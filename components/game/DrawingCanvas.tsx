...
export default function DrawingCanvas({ roomId, isDrawer }: { roomId: string; isDrawer: boolean }) {
    ...
    useEffect(() => {
        ...
        // Fill canvas with white on initial load just to match background completely
        const ctx = ctxRef.current;
        if (ctx) {
                            ...
                            {COLORS.map((c, index) => {
                                ...
                                return (
                                    <button
                                        key={index}
                                        onClick={() => setColor(c)}
                                        className={`w-5 h-5 sm:w-6 sm:h-6 transition-transform relative ${
                                            isSelected ? 'ring-2 ring-white z-10 scale-110 rounded-sm' : 'hover:scale-105 rounded-sm'
                                        }`}
